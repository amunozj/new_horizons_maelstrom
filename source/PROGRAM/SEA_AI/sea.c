...
// <-- KK
	...
	if (sIslandID != "")
	{
		...
		// if(!bstorm) Sea.MaxSeaHeight = 15.0; // screwface : allow big waves around island in storm conditions
...
{
	if (!bSeaActive) return   5.0; // ??????? ????? ??? ????, ??? ????????? pchar.Ship.Pos.x
	if (bSeaActive) return   50.0;
