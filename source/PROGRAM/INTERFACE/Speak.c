...
void InitInterface_RIS(string iniName, aref chr, int whospeak, string sMode)
{
	...
	switch(sMode)
	{
		/*  No longer does a ship spontainously ask this, they just raise the white flag
		 case "capture":
			AskForCapture();
		break;
		*/
...
void AskForTax()
{
	...
	ref refOwnShip = GetShipByType(GetCharacterShipType(pchar));
...
string SelectRumourExSpecial(string rIsland, string rNation)
{
