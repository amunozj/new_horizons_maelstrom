...
void InitInterface(string iniName)
{
	...
	CreateString(true, "ScreenTitle", XI_ConvertString("titleSelectStoryline"), FONT_TITLE, COLOR_NORMAL, 320, 4, SCRIPT_ALIGN_CENTER, 0.9);
	CreateString(true, "StorylineTitle", GetTranslatedStoryLine(slno, GetStorylineTitle(slno)), FONT_NORMAL, COLOR_NORMAL, 320, 84, SCRIPT_ALIGN_CENTER, 1.0); //MAXIMUS 17.05.2019: needed for localization
	...
	CreateTextbox(true, "CharacterName", "CHARNAME_TEXTBOX", "BOX1", "RAMKA1", "", 124, 378, 1.0);
	CreateTextbox(true, "ShipName", "SHIPNAME_TEXTBOX", "BOX2", "RAMKA2", "", 516, 378, 1.0);
...
void ChangeStoryline(bool bRight)
{
	...
	{
		case 0:
			CreateString(true, "HelpText", XI_ConvertString("SelectStorylineFreePlayOnly"), INTERFACE_COAS, COLOR_GREEN_LIGHT, 320, 470, SCRIPT_ALIGN_CENTER, 0.8);
		break;
		case 1:
			CreateString(true, "HelpText", XI_ConvertString("SelectStorylineFreeOpening"), INTERFACE_COAS, COLOR_GREEN_LIGHT, 320, 470, SCRIPT_ALIGN_CENTER, 0.8);
		break;
		case 2:
			CreateString(true, "HelpText", XI_ConvertString("SelectStorylineLocked"), INTERFACE_COAS, COLOR_MONEY, 320, 470, SCRIPT_ALIGN_CENTER, 0.8);
		break;
		case 3:
			CreateString(true, "HelpText", XI_ConvertString("SelectStorylineNoFreePlay"), INTERFACE_COAS, COLOR_RED_LIGHT, 320, 470, SCRIPT_ALIGN_CENTER, 0.8);
		break;
		CreateString(true, "HelpText", XI_ConvertString("SelectStorylineCustomize"), INTERFACE_COAS, COLOR_NORMAL, 320, 470, SCRIPT_ALIGN_CENTER, 0.8);
	}
	...
	if (CheckAttribute(arstart, "model")) {
		...
		GameInterface.strings.CharacterName = GetTranslatedStoryLine(slno, CharFirstName);
		if (CharMiddleName != "") GameInterface.strings.CharacterName = GameInterface.strings.CharacterName + " " + GetTranslatedStoryLine(slno, CharMiddleName);
	...
	{
		curShip = GetShipIndex(arstart.ship);
		if (CheckAttribute(arstart, "shipname"))	GameInterface.strings.ShipName = GetTranslatedStoryLine(slno, arstart.shipname); //MAXIMUS 17.05.2019: needed for localization
...
void RefreshScreen()
{
	...
	switch (curWindow)
	{
		case WINDOW_MAIN:
			...
			if(GetAttribute(GameInterface, "character") == true)
			{
				...
				if (slno == FindStoryline("FreePlay"))
				{
					...
					{
						GameInterface.strings.ShipName = GetTranslatedStoryLine(slno, model.shipname);
					}
				}
			...
			if (curShip >= 0) {
				...
				SendMessage(&GameInterface, "ls", MSG_INTERFACE_REFRESH_SCROLL, "SHIP");
				CharShipName = GetTranslatedStoryLine(slno, GameInterface.strings.ShipName); //MAXIMUS 17.05.2019: needed for localization
			...
			if (CheckAttribute(arstart, "location")) {
				...
				if (slno == FindStoryline("PeterBlood"))
				{
					id = TranslateString("", "Oglethorpe's Estate, England");
		...
		case WINDOW_CHARACTER:
			...
			id = "";
			if (CheckAttribute(model, "name")) id += GetTranslatedStoryLine(slno, model.name);
			if (CheckAttribute(model, "middlename")) {
				if (id != "") id += " ";
				id += GetTranslatedStoryLine(slno, model.middlename);
			}
			if (CheckAttribute(model, "lastname")) {
				if (id != "") id += " ";
				id += GetTranslatedStoryLine(slno, model.lastname);
			}
			if (id != "")
				SendMessage(&GameInterface, "lslsssllllllfl", MSG_INTERFACE_MSG_TO_NODE, "WINDOWSTRINGES", 0, "OutfitName", id, FONT_NORMAL, 380, 140, COLOR_NORMAL, 0, SCRIPT_ALIGN_CENTER, true, 1.0, 0);
			else
			...
			SendMessage(&GameInterface, "lsl", MSG_INTERFACE_MSG_TO_NODE, "SELECT_DESCRIPTION", 35);
		...
		case WINDOW_SHIP:
			...
			if (tmpShip >= 0) {
				...
				if (DEBUG)
					SendMessage(&GameInterface, "lslsssllllllfl", MSG_INTERFACE_MSG_TO_NODE, "WINDOWSTRINGES", 0, "ShipTypeName", TranslateString("", model.id), FONT_SEADOGS, 380, 140, COLOR_NORMAL, 0, SCRIPT_ALIGN_CENTER, true, 1.0, 0);
				else
					SendMessage(&GameInterface, "lslsssllllllfl", MSG_INTERFACE_MSG_TO_NODE, "WINDOWSTRINGES", 0, "ShipTypeName", XI_ConvertString(model.SName), FONT_NORMAL, 380, 130, COLOR_NORMAL, 0, SCRIPT_ALIGN_CENTER, true, 1.0, 0);
...
void GiveHimName(ref char)
{
		...
		CharMiddleName = GetTranslatedStoryLine(slno, CharMiddleName);
		CharLastName = GetTranslatedStoryLine(slno, CharLastName);
}
//MAXIMUS 20.06.2019: needed for localization <==
...
void SetWindow(int iWindow)
{
	...
	switch (curWindow)
	{
		...
		case WINDOW_CHARACTER:
			...
			SendMessage(&GameInterface, "lslsssllllllfl", MSG_INTERFACE_MSG_TO_NODE, "WINDOWSTRINGES", 0, "Title", StringCheckCase(XI_ConvertString("select character"), FONT_TITLE, true), FONT_TITLE, 320, 107, COLOR_NORMAL, 0, SCRIPT_ALIGN_CENTER, true, 0.8, 0);
...
bool SelectMatchingModel(bool bNext)
{
	...
	if (!CheckAttribute(rName, sTmp)) {
		if (CheckAttribute(model, "name") || CheckAttribute(model, "lastname")) {
			rName.(sTmp) = "";
			if (CheckAttribute(model, "name")) rName.(sTmp) = GetTranslatedStoryLine(slno, model.name);
			if (CheckAttribute(model, "middlename")) {
				if (rName.(sTmp) != "") rName.(sTmp) = rName.(sTmp) + " ";
				rName.(sTmp) = rName.(sTmp) + GetTranslatedStoryLine(slno, model.middlename);
			}
			if (CheckAttribute(model, "lastname")) {
				if (rName.(sTmp) != "") rName.(sTmp) = rName.(sTmp) + " ";
				rName.(sTmp) = rName.(sTmp) + GetTranslatedStoryLine(slno, model.lastname);
			}
		}
	}
	GameInterface.character = true; // PB
...
void NavyLockShipSelection(bool LockShip, ref model)
{
	...
	else
	{
		...
		if (curShip == -1)
		{
			...
			GameInterface.SHIP.pic1.tex2 = 1 + GetShipTexture(curShip); // PB
			SendMessage(&GameInterface, "ls", MSG_INTERFACE_REFRESH_SCROLL, "SHIP");
			GameInterface.strings.ShipName = GetTranslatedStoryLine(slno, arstart.shipname);
		}
	}
}