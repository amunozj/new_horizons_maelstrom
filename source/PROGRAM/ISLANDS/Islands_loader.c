...
void CheckIslandChange()
{
	...
	if (nextisland != FindIsland(worldMap.island))
	{
		...
		int enemydist = 0;
		int nextenemy = 0;
		int enemyDistLimit   = 3500;
		int neutralDistLimit = 3500;
		int friendlyDistLimit = 3500;
		...
		// Jan 07, same for friendly ships
		nextenemy = FindClosestShipofRel(GetMainCharacterIndex(), &enemydist, RELATION_FRIEND);
		if (DIRECTSAILDEBUG) trace("DirectsailCheck; next neutral ship: "+nextenemy + " dist: "+enemydist);
		if(nextenemy!= -1 && enemydist<friendlyDistLimit && Characters[nextenemy].ship.type != SHIP_FORT ) // LDH added fort check 08Jan09
		{
		  if (DIRECTSAILDEBUG) trace("Directsail aborted due to neutral ship, dist = " + enemydist);	// LDH added logit to trace - 07Jan09
		  return;
		}
