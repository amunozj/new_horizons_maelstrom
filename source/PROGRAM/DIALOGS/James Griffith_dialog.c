void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "trade_1":
			...
			DialogExit();
			PChar.storekeeper.Idx = sti(NPChar.index);//MAXIMUS
			LaunchStore(GetCurrentStore(NPChar.location)); // KK
		break;
		...
		case "generate_quest":
			if(!CheckAttribute(npchar,"quest.generate_trade_quest.trade_date")) npchar.quest.generate_trade_quest.trade_date = ""; // added by MAXIMUS
			if(npchar.quest.generate_trade_quest.trade_date != lastspeak_date)
			{
				npchar.quest.generate_trade_quest.trade_date = lastspeak_date;
