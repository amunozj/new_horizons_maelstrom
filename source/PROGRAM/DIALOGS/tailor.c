void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "fetch quest complete":
			...
			//Levis Unlock Perk
			if(CheckCharacterPerkLocked(Pchar, "AdvancedCommerce")) UnlockPerkCharacter(PChar ,"AdvancedCommerce");
			//Questbook
			questbookname = "fetch&number="+NPChar.fetch_quest.cargoid+NPChar.index; //Set a questname
		...
		case "agree fetch":
			...
			/*WriteNewLogEntry(("Helping the tailor of "+ctown.name), ("The tailor of "+ctown.name+" asked me to get "+NPChar.fetch_quest.amount+" "+ Goods[sti(NPChar.fetch_quest.good)].name +" from somewhere and deliver it back to him to get "+NPChar.fetch_quest.money+" gold coins. But I have to deliver it before "+NPChar.fetch_quest.expire+" or i will receive nothing."), "Personal", true);
			Logit("Ships Log Updated");
			AddMsgToCharacter(PChar,MSGICON_LOGBOOK);
			PlaySound("INTERFACE\notebook.wav");*/
			//Questbook
			Preprocessor_AddQuestData("town",ctown.name);
			Preprocessor_AddQuestData("type",XI_ConvertString("tailor"));
		...
		case "First time":
			cargoid = getFetchQuestCargo("tailor",ctown);
			if(cargoid != "")
			{
