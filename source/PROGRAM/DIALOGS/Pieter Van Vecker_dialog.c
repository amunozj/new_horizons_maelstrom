	...
	switch(Dialog.CurrentNode)
	{
		...
		case "generate_quest_2":
			...
			else
			{
				...
				dialog.text = DLG_TEXT[42] + GetMyAddressForm(NPChar, PChar, ADDR_GENDER, false, false) + "."; // DeathDaisy: changed "young man" to "my good man/woman", player isn't always young
				link.l1 = DLG_TEXT[43];
				link.l1.go = "exit";
				ChangeCharacterReputation(pchar, 1);
				TradeQuestDone();
