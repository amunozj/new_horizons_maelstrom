	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			else
			{
				...
				dialog.snd2 = "";
				dialog.snd3 = "";
				if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
				else d.Text = RandPhrase(DLG_TEXT[13] + GetMyName(Pchar) + DLG_TEXT[14], DLG_TEXT[15] + GetMyName(Pchar) + DLG_TEXT[16] + TimeGreeting() + DLG_TEXT[17], DLG_TEXT[18] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + " " + GetMyName(PChar) + DLG_TEXT[19], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				if(Rand(1)==0)
				{
				...
				if(CheckAttribute(NPChar, "groot_set") && CheckCharacterItem(PChar, "potionrum"))
				{
					Link.l10 = DLG_TEXT[174];
					Link.l10.go = "cancel_groot";
				}
				else
				{
					if(Rand(2)==0)
					{
						Link.l10 = DLG_TEXT[170];
						Link.l10.go = "set_groot";
					}
				}
			}
		...
		case "Meeting":
				...
				dialog.snd3 = "";
				if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
				else d.Text = RandPhrase(DLG_TEXT[46] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[47], DLG_TEXT[48], DLG_TEXT[49], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				if(Rand(1)==0)
				{
		...
		case "rumours":
				if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
				else d.Text = SelectRumour(FindIslandByLocation(LoadedLocation.id), GetNationIDByType(GetCurrentLocationNation()) );
				Link.l1 = DLG_TEXT[63];
		...
		case "new question":
				...
				dialog.snd3 = "";
				if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
				else d.Text = RandPhrase(DLG_TEXT[65] + GetMyName(Pchar) + DLG_TEXT[66], DLG_TEXT[67] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + " " + GetMyName(PChar) + DLG_TEXT[68], DLG_TEXT[69] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[70], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				if(Rand(1)==0)
				{
		...
		case "info":
				...
				dialog.snd3 = "";
				if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
				else d.Text = RandPhrase(DLG_TEXT[84] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[85], DLG_TEXT[86], DLG_TEXT[87], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				if(Rand(1)==0)
				{	
					Link.l1 = DLG_TEXT[88];
					Link.l1.go = "governor";
...
//<-- RobC/A_S 'smiths
		...
		case "town":
			...
			dialog.snd3 = "";
			if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
			else d.Text = RandPhrase(DLG_TEXT[105] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[106], DLG_TEXT[107], DLG_TEXT[108] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[109], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
...
//<-- RobC/A_S Smiths
		...
		case "town_tavern":
			...
			DeleteAttribute(&locations[FindLocation("Philipsburg_town_02")], "reload.l2.goto_disable"); // JRH: Unlock Fast Travel
		break;

		case "town_shipyard":
			if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
			else d.Text = DLG_TEXT[117];
			...
			Link.l3.go = "exit";
			DeleteAttribute(&locations[FindLocation("Philipsburg_town")], "reload.l4.goto_disable"); // JRH: Unlock Fast Travel
		break;

		case "town_store":
			if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
			else d.Text = DLG_TEXT[121];
			...
			Link.l3.go = "exit";
			DeleteAttribute(&locations[FindLocation("Philipsburg_town_02")], "reload.l3.goto_disable"); // JRH: Unlock Fast Travel
		break;
		...
		case "colony":
			...
			dialog.snd3 = "";
			if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
			else d.Text = RandPhrase(DLG_TEXT[125], DLG_TEXT[126] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[127], DLG_TEXT[128] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[129], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
			if(Rand(1)== 0)
			{
		...
		case "Philipsburg":
			if(CheckAttribute(NPChar, "groot_set")) d.text = DLG_TEXT[173];
			else d.Text = DLG_TEXT[134] + DLG_TEXT[168] + GetTownSize("Philipsburg") + DLG_TEXT[169];// NK
		...
		case "set_groot":	// GR: initiate "I am Groot" Easter egg
			d.Text = DLG_TEXT[171];
			Link.l1 = DLG_TEXT[172];
			Link.l1.go = "exit";
			NPChar.groot_set = true;
		...
		case "cancel_groot":	// GR: in case someone gets too annoyed by "I am Groot"
			...
			Link.l1.go = "exit";
			DeleteAttribute(NPChar, "groot_set");
