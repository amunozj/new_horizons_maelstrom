void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "generate_quest":
			...
			{
				npchar.quest.trade_date = ""; // added by MAXIMUS
			}
		...
		case "generate_quest_2":
			...
			else
			{
				...
				dialog.text = DLG_TEXT[42] + GetMyAddressForm(NPChar, PChar, ADDR_POLITE, false, false) + "."; // DeathDaisy: changed "young man" to sir/maam/monsieur
				link.l1 = DLG_TEXT[43];
				link.l1.go = "exit";
				ChangeCharacterReputation(pchar, 1);
				TradeQuestDone();
