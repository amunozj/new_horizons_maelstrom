void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "fetch quest complete":
			...
			if(!CheckAttribute(PChar,"fetchquestcompleted_apothecary")) PChar.fetchquestcompleted_apothecary = 0;
			PChar.fetchquestcompleted_apothecary = sti(PChar.fetchquestcompleted_apothecary) + 1;
		break;
		...
		case "agree fetch":
			...
			Preprocessor_AddQuestData("type",XI_ConvertString("apothecary"));
		...
		case "First time":
			cargoid = getFetchQuestCargo("apothecary",ctown);
			if(cargoid != "")
			{
