void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "fetch quest complete":
			...
			//Levis Unlock Perk
			if(CheckCharacterPerkLocked(Pchar, "ImproveAiming")) UnlockPerkCharacter(PChar ,"ImproveAiming");
			//Questbook
			questbookname = "fetch&number="+NPChar.fetch_quest.cargoid+NPChar.index; //Set a questname
			...
			if(!CheckAttribute(PChar,"fetchquestcompleted_gunsmith")) PChar.fetchquestcompleted_gunsmith = 0;
			PChar.fetchquestcompleted_gunsmith = sti(PChar.fetchquestcompleted_gunsmith) + 1;
		...
		case "agree fetch":
			...
			Preprocessor_AddQuestData("type",XI_ConvertString("gunsmith"));
		...
		case "SelectFix":
			cargoid = getFetchQuestCargo("gunsmith",ctown);
			if(cargoid != "")
			{
