...
*/
		...
		case "slave_camp_fight_won":
			...
			{
				survivors = survivors + 1;
				crewref = characterFromID("Amt�nio Guajira");
		...
		case "move_out":
			...
			if (!LAi_IsDead(characterFromID("Amt�nio Guajira")))
			{
				...
				LAi_group_MoveCharacter(characterFromID("Amt�nio Guajira"), LAI_GROUP_PLAYER);
			}
			if (!LAi_IsDead(characterFromID("Coper Blumberger")))
			{
				LAi_SetActorType(characterFromID("Coper Blumberger"));
...
// Doolin Becart should now be standing on Cape Francos waiting for you to talk to him.  His dialog exits to either "sail_with_doolin" or "steal_doolins_boat".
		...
		case "kidnap_escape2":
			...
			Locations[FindLocation("Redmond_town_03")].reload.l9.disable = 1; // If you have kidnapped Luc�a / Edmundo then you won't get to do the courier jobs
		break;
...
//			LAi_ActorTurnToLocator(characterFromID("Javier Balboa"), "reload", "reload1");  // Governor will now wait until you ask him for ransom
		...
		case "governors_revenge_fetch_romance":	// Triggered by dialog with "Javier Balboa" if you ransomed Luc�a / Edmundo, rescued them from Downing dungeon, and don't have them as officer
...
//			EquipCharacterByItem(ch,"blade33");
		...
		case "abduction_officer_setup":
			...
			Locations[FindLocation("Cuba_port")].vcskip = true;	// Otherwise too many random people in Havana means Luc�a / Edmundo can't spawn
			Locations[FindLocation("Cuba_shore_02")].vcskip = true;	// Otherwise Luc�a / Edmundo strolls past random bandits at Bahia de Moa
...
//			LAi_SetOfficerType(romance);
		...
		case "convoy_Willemstad_Shipyard":
			SetCharacterRemovable(romance, true); // In case you put Luc�a / Edmundo in command of a ship
...
//			PChar.Ship.crew.quantity = MakeInt(GetMaxCrewQuantity(PChar) / 10);
		...
		case "convoy_final_briefing":
			SetModelFromID(romance, PChar.quest.romance_model);
			DeleteQuestAttribute("romance_model");
			if (!isofficer(romance)) SetOfficersIndex(Pchar, 3, romanceidx);	// Just in case you've somehow managed to replace Luc�a / Edmundo
			ChangeCharacterAddressGroup(romance, GetCharacterShipCabin(PChar), "rld", "startloc");
...
//			characters[GetCharacterIndex(PChar.quest.mona_attack.fleet_commander)].Dialog.Filename = PChar.quest.normal_dialog;
		...
		case "treachery_villain_invites_romance":	// Triggered by dialog with Piers / Valerie Downing if Luc�a / Edmundo is with you
...
//			LAi_ActorDialog(romance,PChar,"treachery_romance_rejects_villain",5.0,5.0); // Doesn't seem to lead to next quest case if 'AddDialogExitQuest' present in dialog
		...
		case "threat_goto_santiago": // Triggered by dialog with Luc�a / Edmundo if present when you visit townhall
...
//			LAi_type_actor_Reset(PChar);
		...
		case "hunt_port_au_prince_arrival":
			...
			bQuestDisableSeaEnter = true;
			Locations[FindLocation("Buccaneers_Camp")].reload.l4.disable = 0;	// Unlock forger's house - not needed if Luc�a or Edmundo is with you but allow it anyway
...
//			DisableFastTravel(false);
		...
		case "hunt_fort_setup":
			...
			ChangeCharacterAddressGroup(CharacterFromID("Amt�nio Guajira"), "PoPrince_fakefort3", "rld", "loc13");
			LAi_SetCitizenType(CharacterFromID("Amt�nio Guajira"));
			...
			RemoveCharacterEquip(CharacterFromID("Amt�nio Guajira"), BLADE_ITEM_TYPE);
			TempRemoveItems(CharacterFromID("Ralph Partridge"));
			TempRemoveItems(CharacterFromID("Coper Blumberger"));
			TempRemoveItems(CharacterFromID("Amt�nio Guajira"));
...
//			characters[GetCharacterIndex(PChar.quest.speaker)].Dialog.Filename = PChar.quest.normal_dialog;
		...
		case "hunt_fort_crew_released":
			...
			LAi_SetActorType(CharacterFromID("Amt�nio Guajira"));
			LAi_ActorFollowEverywhere(CharacterFromID("Amt�nio Guajira"), "", 10.0);
			Pchar.quest.hunt_fort_to_town.win_condition.l1 = "location";
			Pchar.quest.hunt_fort_to_town.win_condition.l1.location = "PoPrince_town";
			PChar.quest.hunt_fort_to_town.win_condition = "hunt_out_of_fort";
			Pchar.quest.hunt_fort_to_jungle.win_condition.l1 = "location";
...
//			Locations[FindLocation("Buccaneers_Camp")].reload.l4.disable = 1;	// Re-lock forger's house
			if (isofficer(romance))					// Also reset "romance" in case Luc�a/Edmundo is still in "follow" mode
			{
				LAi_SetOfficerType(romance); 
				SetCharacterRemovable(romance, true);
			}
...
//			characters[GetCharacterIndex(PChar.quest.speaker)].Dialog.Filename = PChar.quest.normal_dialog;
		...
		case "hunt_back_to_ship":
			...
			RestoreTempRemovedItems(CharacterFromID("Amt�nio Guajira"));
			LAi_SetCitizenType(CharacterFromID("Ralph Partridge"));
			LAi_SetCitizenType(CharacterFromID("Coper Blumberger"));
			LAi_SetCitizenType(CharacterFromID("Amt�nio Guajira"));
			ChangeCharacterAddress(characterFromID("Ralph Partridge"), "None", "");
...
// PChar.quest.rescuer = "Grigorio Formoselle";	// for testing
		...
		case "imperial_escort_Santo_Domingo_attack":
			...
			Characters[GetCharacterIndex("Jaime Lezcano")].model.animation = "man"
...
//			LAi_SetCitizenType(CharacterFromID("Warden"));
		...
		case "imperial_escort_admiral_leaves2":
			...
			NPChar = CharacterFromID("Emiliano de Guzm�n");
			...
			Towns[GetTownIndex("Santiago")].gov = "Emiliano de Guzm�n";
			Locations[FindLocation("Santiago_town_01")].reload.l4.disable = 1;	// Lock Santiago townhall so you can't face Emiliano de Guzm�n until the right time
...
//			LAi_group_SetCheck("Imperial_Escort_Agent", "imperial_escort_PoP_ambush_battle_won");
		...
		case "imperial_escort_agent_captured":
			...
			Preprocessor_AddQuestData("evilgov", GetMyFullName(CharacterFromID("Emiliano de Guzm�n")));
			Preprocessor_AddQuestData("governor", GetMyFullName(CharacterFromID("Javier Balboa")));
			Preprocessor_AddQuestData("envoy", GetMyFullName(CharacterFromID("Imperial_envoy")));
...
//			if(CheckQuestAttribute("imperial_escort.status", "ship_missing"))
		...
		case "imperial_escort_havana_for_ether":
			...
			for(i=1; i<OFFICER_MAX; i++)				// Find which slot Luc�a/Edmundo is occupying in case you manage to displace them by recruiting a new officer
			{
		...
		case "imperial_escort_havana_arrival_for_ether":
			...
			DisableFastTravel(true);
			if(!IsOfficer(romance) || romance.location != PChar.location)	// If Luc�a/Edmundo has somehow been displaced, put them back
			{
...
//			CreateParticleSystem("cancloud" , 9.58, -3.19, -12.24, 0.0, 0.0, 0.0, sti(20) );	// Based on "reload1"
		...
		case "imperial_escort_after_rescue_arrest":
			ChangeCharacterAddressGroup(CharacterFromID("Emiliano de Guzm�n"), "Santiago_townhall", "goto", "goto3");
			LAi_SetHuberStayType(CharacterFromID("Emiliano de Guzm�n"));
			TakeItemFromCharacter(CharacterFromID("Emiliano de Guzm�n"), FindCharacterItemByGroup(CharacterFromID("Emiliano de Guzm�n"), BLADE_ITEM_TYPE));
			if (ENABLE_WEAPONSMOD)
			{
				GiveItem2Character(CharacterFromID("Emiliano de Guzm�n"),"bladeC17+2");
				EquipCharacterByItem(CharacterFromID("Emiliano de Guzm�n"),"bladeC17+2");
			}
			else
			{
				GiveItem2Character(CharacterFromID("Emiliano de Guzm�n"),"bladeC17");
				EquipCharacterByItem(CharacterFromID("Emiliano de Guzm�n"),"bladeC17");
			}
			if (!CheckCharacterItem(CharacterFromID("Emiliano de Guzm�n"), "goldarmor"))
			{
				GiveItem2Character(CharacterFromID("Emiliano de Guzm�n"), "goldarmor");
				EquipCharacterByItem(CharacterFromID("Emiliano de Guzm�n"), "goldarmor");
			}
			Characters[GetCharacterIndex("Emiliano de Guzm�n")].perks.list.BasicDefence = true;		// Need to assign abilities here because levelling system fouls them up earlier
			Characters[GetCharacterIndex("Emiliano de Guzm�n")].perks.list.AdvancedDefence = true;
			Characters[GetCharacterIndex("Emiliano de Guzm�n")].perks.list.SwordplayProfessional = true;
			Characters[GetCharacterIndex("Emiliano de Guzm�n")].perks.list.CriticalHit = true;
			NPChar = CharacterFromID("Imperial_envoy");
...
//			DeleteQuestAttribute("arresterid");
		...
		case "imperial_escort_after_rescue_onto_townhall4":
			Locations[FindLocation("Santiago_town_01")].reload.l4.disable = 0;	// Unlock Santiago townhall so you can confront Emiliano de Guzm�n
			Locations[FindLocation("Santiago_town_01")].locators_radius.reload.reload4 = 1.0;
			LAi_SetPlayerType(PChar);
			LAi_SetActorType(CharacterFromID("Imperial_envoy"));

			Characters[GetCharacterIndex("Emiliano de Guzm�n")].dialog.CurrentNode = "revenge";
			...
			PChar.quest.imperial_escort_guzman_defeated_1.win_condition.l2.character = "Emiliano de Guzm�n";
...
//			DeleteAttribute(ImpShip, "EmblemedSails.nationFileName");
			...
			LAi_ActorDialogNow(CharacterFromID("Jusepe Guimaraes"), PChar,"imperial_escort_envoy_to_admiral_again",1.0);		// Exits, triggers "imperial_escort_go_get_guzman" if Emiliano de Guzm�n is still alive
...
// See cases "imperial_escort_admiral_leaves2" and "imperial_escort_ships_impounded"
		...
		case "imperial_escort_go_get_guzman":
			...
			Characters[GetCharacterIndex("Emiliano de Guzm�n")].dialog.CurrentNode = "arrest";
...
//			PChar.quest.imperial_escort_guzman_defeated_2.win_condition.l1.character = "Emiliano de Guzm�n";
...
//				PlaceCharacter(sld, "goto");
		...
		case "imperial_escort_soldiers_outside_townhall2":
			...
			PChar.quest.imperial_escort_guzman_defeated_2.win_condition.l2.character = "Emiliano de Guzm�n";
			PChar.quest.imperial_escort_guzman_defeated_2.win_condition = "imperial_escort_guzman_defeated_2";
		break;
		...
		case "imperial_escort_fight_guzman":	// Triggered by dialog with Emiliano de Guzm�n
			LAi_LocationFightDisable(&Locations[FindLocation("Santiago_townhall")], false);
			LAi_SetActorType(CharacterFromID("Emiliano de Guzm�n"));
			LAi_SetImmortal(CharacterFromID("Emiliano de Guzm�n"), false);
			LAi_NoRebirthEnable(CharacterFromID("Emiliano de Guzm�n"));
			LAi_group_MoveCharacter(CharacterFromID("Emiliano de Guzm�n"), LAI_GROUP_MONSTERS);
		...
		case "imperial_escort_restore_santiago_governor":	// Triggered by dialog with Jusepe Guimaraes
			...
			ChangeCharacterAddress(CharacterFromID("Emiliano de Guzm�n"), "none", "");
			ChangeCharacterAddressGroup(NPChar, "Santiago_townhall", "sit", "sit1");
			LAi_SetHuberType(NPChar);
