...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Meeting":
			    if(NPChar.sex == "man")
			    {
					...
					dialog.snd2 = "";
					dialog.snd3 = "";
				   d.Text = RandPhrase(DLG_TEXT[45] + PChar.name + DLG_TEXT[46], DLG_TEXT[47] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + " " + PChar.lastname + DLG_TEXT[48], DLG_TEXT[49] + PChar.name + DLG_TEXT[50], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				}
				else
				{
					dialog.snd1 = "";
					dialog.snd2 = "";
					dialog.snd3 = "";
				   d.Text = RandPhrase(DLG_TEXT[51] + PChar.name + DLG_TEXT[52], DLG_TEXT[53] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + " " + PChar.lastname + DLG_TEXT[54], DLG_TEXT[55] + PChar.name + DLG_TEXT[56], &Dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				}
				if(Rand(1)==0)
				{
		...
		case "town_store":
			d.Text = DLG_TEXT[141] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + " " + PChar.name + DLG_TEXT[142];
	...
	/*		else
		...
		case "Oxbay":
			d.Text = DLG_TEXT[153] + DLG_TEXT[172] + GetTownSize("Oxbay") + DLG_TEXT[173]; // NK
